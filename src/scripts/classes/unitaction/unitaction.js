...
setup.UnitAction = class UnitAction extends setup.TwineClass {
     ...
     */
    constructor({ key, tags, quest_template, prerequisites, unit_requirements, result_traits, prerequisite_traits, is_decrease, is_multitrain, is_allow_injured, }) {
        super();
        // assumes quest has an actor named "trainee"
        // name is leftover from previous uses
        if (!key)
            throw new Error(`null key base for ${key}`);
        this.key = key;
        if (!Array.isArray(tags))
            throw new Error(`tags for unitaction ${key} must be an array`);
        this.tags = tags;
        if (!quest_template)
            throw new Error(`null quest base for ${key}`);
    ...
    getFinalTraits(unit) {
        ...
        for (const will_get of raw_traits) {
            if (!unit.isTraitCompatible(will_get))
                continue;
