...
setup.UnitActionHelper.populateTraits = function (unit, target_traits) {
    ...
    for (const trait of target_traits.filter(a => true)) {
        ...
        if (trait_group) {
            ...
            if (target_traits.filter(t => t.getTraitGroup() == trait_group &&
...
setup.UnitActionHelper.generateQuest = function (unit, target_traits, raw_forced_units) {
    ...
    if (candidates.length) {
        ...
        candidates.sort((action1, action2) => action1.getTemplate().getDifficulty().getLevel() - action2.getTemplate().getDifficulty().getLevel());
        ...
        if (raw_forced_units &&
            ...
            if (!assignment) {
                setup.notify(`Unable to find an assignment for the ongoing training for a|rep! Please assign the team manually.`, { a: unit });
            }
            else {
                // assign the team
            ...
            setup.notify(`Unable to deploy a team for the ongoing training for a|rep! Please assign the team manually.`, { a: unit });
        }
    }
    else {
