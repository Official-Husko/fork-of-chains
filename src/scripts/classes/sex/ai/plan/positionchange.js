...
class PositionChange extends setup.SexPlan {
    ...
    selectAction(actions) {
        ...
        const possible = actions.filter(action => action instanceof setup.SexAction.PositionChange && action.getNewPosition() == want);
        if (possible.length)
        ...
        if (this.sex.isBeingPenetrated(this.unit)) {
            // stop penetrations
            const possible = actions.filter(action => action.getTags().includes('penetrationendsub'));
            if (possible.length)
                return setup.rng.choice(possible);
            return this.giveUp();
        }
        ...
        if (my_position == setup.sexposition.center && this.sex.getUnitAtPosition(setup.sexposition.top)) {
            ...
            const possible = actions.filter(action => action instanceof setup.SexAction.PositionChange &&
        ...
        if (my_position == setup.sexposition.center && !this.sex.getUnitAtPosition(want)) {
            ...
            const possible = actions.filter(action => action instanceof setup.SexAction.PositionChangeOther &&
