...
setup.SexBodypart = class SexBodypart extends setup.TwineClassCustom {
    ...
    _isCanInteractWithHeight(my_height, their_bodypart, their_height) {
        return Math.abs(my_height.getHeightValue() - their_height.getHeightValue()) <= Math.max(this.heightTolerance(), their_bodypart.heightTolerance());
    }
    /**
    ...
    _isCanInteractWithPositionFacing(my_position, my_facing, their_bodypart, their_position, their_facing) {
        if (!my_position.isAdjacentTo(their_position))
            return false;
        ...
        if (topcenter.includes(my_position) && topcenter.includes(their_position)) {
            ...
            if (my_position == setup.sexposition.top) {
                // I am on top
                // front body matches
                const myflexfront = (this.isDirectionless() ||
                    mp == setup.sexfacing.downfront ||
                    (this.isFlexible() && mp.isFrontIsh()));
                const theirflexfront = (their_bodypart.isDirectionless() ||
                    tp == setup.sexfacing.upfront ||
                    (their_bodypart.isFlexible() && tp.isFrontIsh()));
                if (myflexfront && theirflexfront)
                    return true;
                // back body matches
                const myflexback = (this.isDirectionless() ||
                    mp == setup.sexfacing.downback ||
                    (this.isFlexible() && !mp.isFrontIsh()));
                const theirflexback = (this.isDirectionless() ||
                    tp == setup.sexfacing.upback ||
                    (their_bodypart.isFlexible() && !tp.isFrontIsh()));
                if (myflexback && theirflexback)
                    return true;
            }
            else {
                // I am on bottom
                ...
                const myflexfront = (this.isDirectionless() ||
                    ...
                    (this.isFlexible() && mp.isFrontIsh()));
                const theirflexfront = (this.isDirectionless() ||
        ...
        if (my_position.isLeftOf(their_position)) {
            ...
            const mine = (this.isDirectionless() ||
                mp == setup.sexfacing.back ||
                (this.isFlexible() && !mp.isFrontIsh()));
            const theirs = (their_bodypart.isDirectionless() ||
    ...
    isCanInteractWith(my_position, my_facingheight, their_bodypart, their_position, their_facingheight) {
        return (this._isCanInteractWithHeight(my_facingheight.height, their_bodypart, their_facingheight.height) &&
            this._isCanInteractWithPositionFacing(my_position, my_facingheight.facing, their_bodypart, their_position, their_facingheight.facing));
    }
    /* =========================
        TEXT
    ========================= */
    ...
    describeTease(unit, target, target_bodypart, sex) {
        return setup.SexUtil.convert(this.rawDescribeTease(unit, target, target_bodypart, sex), { a: unit, b: target }, sex);
    }
    /**
    ...
    describeEnd(unit, target, target_bodypart, sex) {
        return setup.SexUtil.convert(this.rawDescribeEnd(unit, target, target_bodypart, sex), { a: unit, b: target }, sex);
    }
    /**
    ...
    verbPenetrate(unit, target, target_bodypart, sex) {
        return setup.SexUtil.convert(this.rawVerbPenetrate(unit, target, target_bodypart, sex), { a: unit, b: target }, sex);
    }
    /**
    ...
    repPenetrateFlavorSentence(unit, target, target_bodypart, sex) {
        return setup.SexUtil.convert(this.rawPenetrateFlavorSentence(unit, target, target_bodypart, sex), { a: unit, b: target }, sex);
    }
    /**
    ...
    static describePenetrationEnds(penetrations, sex) {
        ...
        for (const penetration of penetrations) {
            ...
            sentences.push(penetration.bodypart.describeEnd(penetration.unit, target.unit, target.bodypart, sex));
        }
