...
setup.SexAction = class SexAction extends setup.TwineClassCustom {
    ...
    isAllowed(sex) {
        ...
        if (!setup.RestrictionLib.isPrerequisitesSatisfied(this, this.getRestrictions()))
            return false;
        // check actor restrictions
        const units = this.getUnits();
        for (const unit of units) {
            const description = this.getActorDescription(unit, sex);
            if (!setup.RestrictionLib.isUnitSatisfy(unit, description.restrictions || []))
                return false;
        }
        // check disabled
    ...
    isAIAllowed(main_unit, sex) {
        ...
        if (!permission.isActionAllowed(this, sex))
            return false;
        // everything looks ok
        return true;
    }
    ...
    applyOutcomes(sex) {
        ...
        for (const unit of units) {
            ...
            if (arousal)
                sex.adjustArousal(unit, arousal);
            const discomfort = this.getDiscomfort(unit, sex);
            if (discomfort)
                sex.adjustDiscomfort(unit, discomfort);
            const energy = this.getEnergy(unit, sex);
            if (energy)
                sex.adjustEnergy(unit, energy);
        }
    }
    /**
