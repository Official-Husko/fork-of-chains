...
setup.SexScene = class SexScene extends setup.TwineClass {
    ...
    getPossibleActions(unit) {
        ...
        for (const action_class of all_action_classes) {
            ...
            const permutations = setup.allPermutations(actor_count - 1, units.filter(u => u != unit));
        ...
        const res = all_actions.filter(action => {
            if (!action.isAllowed(this.getSex()))
                return false;
            if (!permission.isActionAllowed(action, this.getSex()))
                return false;
            return true;
        });
    ...
    appendText(text) {
        ...
        setup.DOM.Helper.append(this.description_selector, dom, 
        /* animate = */ true);
    ...
    appendLineBreaks() {
        const height = $(this.description_selector).height();
        setup.DOM.Helper.append(this.description_selector, html `<br/>`, 
        /* animate = */ true);
        const new_height = $(this.description_selector).height();
        document.documentElement.scrollBy(0, new_height - height);
    }
