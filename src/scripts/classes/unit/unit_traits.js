...
setup.Unit.prototype.addTrait = function (trait, trait_group, is_replace) {
    ...
    if (!trait && !trait_group)
        throw new Error(`Must have either non null trait or non null trait group`);
    if (trait) {
        if (trait_group) {
    ...
    // check for blessing of wolf and curse of lamb
    {
        ...
        if (trait_group && [sub, dom].includes(trait_group.getTraits()[0])) {
            if ((new_trait == sub && !this.isHasTrait(sub)) ||
            ...
            else if ((new_trait == dom && !this.isHasTrait(dom)) ||
                ...
                if (current.length >= limit) {
                    ...
                    if (this.isYourCompany() || State.variables.gDebug) {
                        if (new_trait.getTags().includes('perk')) {
                            setup.notify(`a|Rep <<dangertext 'a|fail'>> to gain ${new_trait.rep()} because a|they already a|have too many perks! You can reset a|their perks using the ${setup.item.potion_perk.rep()}.`, { a: this });
                        }
                        else {
                            setup.notify(`a|Rep <<dangertext 'a|fail'>> to gain ${new_trait.rep()} because a|they already a|have too many traits of this same type`, { a: this });
                        }
                    }
                    break;
                }
...
setup.Unit.prototype.getNonInnateSkinTraits = function () {
    ...
    for (const trait of skins) {
        if (!this.isHasInnateTrait(trait))
            result.push(trait);
    }
    return result;
};
...
export function removeMindbrokenTraits(traits) {
    ...
    return traits.filter(trait => !trait.getTags().includesAny(disabled_traits_base)).filter(trait => trait != setup.trait.will_defiant).filter(trait => (trait == setup.trait.training_mindbreak || !trait.getTags().includes('training')));
}
/**
...
setup.Unit.prototype.getTraitFromTraitGroup = function (trait_group) {
    if (!trait_group)
        throw new Error(`missing trait group`);
    const traits = trait_group.getTraits();
    for (const trait of traits) {
        if (trait && this.isHasTraitExact(trait))
            return trait;
    }
    return null;
};
...
setup.Unit.prototype.isHasTraitIncludeExtra = function (trait) {
    for (const trait_covered of trait.getTraitCover()) {
        if (this.isHasTraitIncludeExtraExact(trait_covered))
            return true;
    }
    return false;
};
...
setup.Unit.prototype.isHasTrait = function (trait_raw, trait_group, ignore_cover) {
    ...
    if (!traitgroup) {
        if (!trait)
            throw new Error(`Missing trait: ${trait_raw}`);
...
setup.Unit.prototype.isHasTraitExact = function (trait_raw) {
    ...
    if (!trait)
        throw new Error(`Missing trait in is has trait exact: ${trait_raw}`);
...
setup.Unit.prototype.isHasTail = function (includes_tailplug) {
    if (this.getTail())
        return true;
    if (includes_tailplug && this.getTailPlug())
        return true;
    return false;
};
/**
...
setup.Unit.prototype.getTraitWithTag = function (tag) {
    ...
    for (let i = 0; i < traits.length; ++i) {
        if (traits[i] && traits[i].getTags().includes(tag))
            return traits[i];
    }
    return null;
};
...
setup.Unit.prototype.getSubrace = function () {
    ...
    const traits = Object.keys(this.trait_key_map).map(trait_key => setup.trait[trait_key]).filter(trait => trait.getTags().includes('subrace'));
    if (traits.length > 1) {
...
setup.Unit.prototype.getRace = function () {
    ...
    const main_races = subrace.getTags().map(key => setup.trait[key]).filter(trait => trait && trait.getTags().includes('race'));
...
setup.Unit.prototype._getPurifiable = function (trait_tag) {
    ...
    for (const trait of candidates) {
        // wrong tag = continue
        if (trait_tag && !trait.getTags().includes(trait_tag))
            continue;
        // dicks only for units with dicks
        if (trait.getTags().includes('dickshape') && !has_dick)
            continue;
...
setup.Unit.prototype.purify = function (trait_tag) {
    ...
    if (this.getRace() == setup.trait.race_demon) {
        ...
        if (this.isYourCompany()) {
            setup.notify(`a|Rep a|attempt to be purified but demons cannot be purified, no matter what`, { a: this });
        }
        return null;
    }
 ...
 * @returns {setup.Trait | null}
 */
function doCorrupt({ unit, trait_tag, is_return_anyways, is_ignore_blessing, }) {
    {
...
setup.Unit.prototype.getSpeechChances = function () {
    // {speechkey: score}
    let chances = {};
    for (let speechkey in setup.speech) {
        let speech = setup.speech[speechkey];
...
setup.Unit.prototype.isAllowedTalk = function () {
    ...
    if (this.isSlave() && bedchamber && bedchamber.getOption('speech') != 'full')
        return false;
    return true;
};
...
setup.Unit.prototype.isCanOrgasm = function () {
    if (!this.isAllowedOrgasm())
        return false;
    if (this.isHasTrait(setup.trait.eq_chastity))
        return false;
    if (this.isHasDick() && !this.isCanPhysicallyCum())
        return false;
    return true;
};
