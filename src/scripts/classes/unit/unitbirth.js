...
function doGenerateChild(father, mother) {
    ...
    if (Math.random() < setup.CHILD_TRAIT_BACKGROUND_INHERIT_CHANCE) {
        const father_backgrounds = father.getAllTraitsWithTag('bg').filter(trait => father.isHasRemovableTrait(trait));
        const mother_backgrounds = mother.getAllTraitsWithTag('bg').filter(trait => mother.isHasRemovableTrait(trait));
    ...
    // inherit traits
    const traits = father.getInheritableTraits().concat(mother.getInheritableTraits()).concat(base_unit.getInheritableTraits());
    ...
    for (const trait of traits) {
        if (!base_unit.isTraitCompatible(trait))
            continue;
        const tags = trait.getTags();
        // breast is an exception
        if (tags.includes('breast') && base_unit.isMale())
            continue;
