...
setup.UnitImage = class UnitImage extends setup.TwineClass {
    ...
    _getDeepestNode(unit, node) {
        if (!node)
            node = setup.UnitImage.UNIT_IMAGE_TABLE;
    ...
    getLastUsed(image) {
        if (!(image.path in this.image_last_used))
            return -setup.UNIT_IMAGE_MEMORY;
    ...
    getRandomImage(unit) {
        ...
        function ImageCmp(image1, image2) {
            ...
            if (!stale1 && stale2)
                return 1;
            // next is via depth
            if ((image1.depth ?? 0) < (image2.depth ?? 0))
                return -1;
            if ((image2.depth ?? 0) < (image1.depth ?? 0))
                return 1;
            // via last use...
            if (last_use1 > last_use2)
                return -1;
            if (last_use1 < last_use2)
                return 1;
            // via index
    ...
    static initializeImageTable(is_initial) {
            ...
            return importScripts(directory + 'imagemeta.js').then(function () {
                ...
                */
                ...
                for (const traitkey of further) {
                    ...
                    // warning: this is asynchronous:
                    promises.push(processImageMeta(next_dir, further_out, packmeta, depth + 1, is_packmeta_only, imagepack));
                }
        ...
        const tryLoadImagePack = (imagepack, is_skip_images) => {
            ...
            return new Promise((resolve, reject) => {
                importScripts(imagepack + '/imagepack.js')
                    ...
                    processImageMeta(imagepack + '/', output_node, packmeta, 
                    /* depth = */ 1, is_skip_images, imagepack).then(resolve, reject);
                });
            }).then(() => packmeta);
        };
        ...
        return Promise.allSettled(imagepacks.map(imagepack => tryLoadImagePack(imagepack, !current.includes(imagepack)).then((packmeta) => {
