...
setup.DOM.Card.bedchamber = function (bedchamber, hide_actions, show_remove_button) {
    ...
    // slaves
    {
      ...
      `);
        ...
        inner.push(setup.DOM.Util.message(`(Rules)`, () => {
    ...
    // furniture grid
    {
        ...
        for (const slot of Object.values(setup.furnitureslot)) {
            ...
            if (show_remove_button && !furniture.isBasic()) {
                very_inner.push(html `
          ${setup.DOM.Util.message(`(remove)`, removeFurnitureCallback(slot, bedchamber))}
          `);
            }
    ...
    return setup.DOM.create('div', { class: 'equipmentsetcard card' }, fragments);
};
