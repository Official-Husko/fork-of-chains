...
setup.DOM.Card.team = function (team, hide_actions) {
    const fragments = [];
    fragments.push(setup.DOM.Util.menuItemToolbar(teamNameActionMenu(team, hide_actions)));
    const quest = team.getQuest();
    if (quest) {
        const inner = [];
        inner.push(html `
    ...
    for (const unit of team.getUnits()) {
      ...
      <div>
        ...
        ${unit.repLong()}
        ${!hide_actions && !team.getQuest() && setup.DOM.Nav.link(`(remove)`, removeUnitCallback(team, unit))}
      </div>
    `);
    }
