...
setup.DOM.Card.unit = function (unit, hide_actions) {
  ...
  <span class='unitimage'>
    ${setup.DOM.Util.onEvent('click', setup.DOM.Util.Image.load({ image_name: unit.getImage(), fallback: setup.UnitImage.DEFAULT_IMAGE.path }), () => {
  ...
  `);
    now_fragments.push(setup.DOM.Util.async(() => {
        ...
        /* Titles */
        {
          ...
          ${setup.DOM.Util.message(`(+${all_titles.length - titles.length})`, () => {
                    return html `
                <div class='helpcard'>
                  ${setup.Text.replaceUnitMacros(`a|Rep also a|have the following titles, but these titles are inactive`, { a: unit })}
        ...
        `);
                skill_fragments.push(setup.DOM.Util.help(html `
        ...
        `);
            ...
            fragments.push(setup.DOM.create('div', {}, misc_fragments_with_separator));
        }
