...
setup.DOM.Card.party = function (party, hide_actions) {
    const fragments = [];
    fragments.push(setup.DOM.Util.menuItemToolbar(partyNameActionMenu(party, hide_actions)));
    for (const unit of party.getUnits()) {
        fragments.push(html `
      <div>
        ${setup.DOM.Util.level(unit.getLevel())}
        ${unit.repLong()}
        ${!hide_actions && setup.DOM.Nav.link(`(remove)`, removeUnitCallback(party, unit))}
      </div>
    `);
    }
