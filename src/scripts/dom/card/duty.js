...
setup.DOM.Card.duty = function (duty, hide_actions) {
    ...
    const fragments = [];
    const unit = duty.getAssignedUnit();
    fragments.push(setup.DOM.Util.menuItemToolbar(dutyNameActionMenus(duty, hide_actions)));
    ...
    if (template.isHasPrestigeAmount() || template.isHasTriggerChance()) {
        ...
        if (Object.keys(traits).length) {
            inner_fragments.push(setup.DOM.Util.message(`(full details)`, () => { return dutyFullDetails(duty, unit); }));
        }
        fragments.push(setup.DOM.create('div', {}, inner_fragments));
    }
    if (!hide_actions && State.variables.menufilter.get('duty', 'display') == 'shortened') {
        fragments.push(setup.DOM.Util.message(`(description)`, () => dutyDescriptionFragment(duty)));
    }
    else {
