...
setup.DOM.Card.unitskill = function (unit, skill) {
  ...
  `);
    ...
    fragments.push(delineate_fragments(base_breakdown.map(breakdown => html `${breakdown.title}: ${format_skill(breakdown.value)}`)));
  ...
  `);
    ...
    fragments.push(delineate_fragments(mod_breakdown.map(breakdown => html `${breakdown.title}: ${setup.DOM.Text.percentage(breakdown.value)}`)));
    fragments.push(html `<hr/>`);
  ...
  `);
    ...
    fragments.push(delineate_fragments(add_breakdown.map(breakdown => html `${breakdown.title}: ${format_skill(breakdown.value)}`)));
