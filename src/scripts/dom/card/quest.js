...
function getQuestUnitRolesFragment(quest) {
    return setup.DOM.create('span', {}, Object.values(quest.getUnitCriteriasList()).map(actor_obj => getQuestUnitRoleFragment(actor_obj)));
}
/**
 * @param {setup.QuestInstance} quest
 * @returns {setup.DOM.Node}
    ...
    if (show_hide_button) {
        menus.push(menuItemAction({
            ...
            callback: () => {
                ...
                setup.DOM.Helper.replace(selector, questCardCompactInternal(quest, hide_actions));
            },
        }));
    }
    if (show_open_button) {
        menus.push(menuItemAction({
            ...
            callback: () => {
                const selector = `#${getCompactDivId(quest)}`;
                setup.DOM.Helper.replace(selector, setup.DOM.Card.quest(quest, hide_actions));
            },
        }));
    }
...
function getQuestExtraActorsFragment(quest) {
    ...
    if (!actors.length)
        return null;
    return setup.DOM.create('div', {}, [html `Involved: `].concat(actors.map(unit => unit.repLong())));
}
/**
 * @param {setup.QuestInstance} quest
 * @returns {setup.DOM.Node}
...
function questCardCompactInternal(quest, hide_actions) {
    ...
    const fragments = [];
    fragments.push(setup.DOM.Util.menuItemToolbar(questNameActionMenu(quest, hide_actions, 
...
function getQuestDescriptionFragment(quest) {
    ...
    if (description_display == 'hidden' ||
        ...
        return html `
  ${setup.DOM.Util.message('(description)', () => renderDescription(quest, quest.getDescriptionPassage()))}`;
    }
    else {
...
setup.DOM.Card.quest = function (quest, hide_actions) {
    ...
    // async loading
    fragments.push(setup.DOM.Util.async(() => { return getQuestCardAsyncFragment(quest, hide_actions); }));
    // debug actions
    if (State.variables.gDebug) {
        fragments.push(html `
    <div>
