...
function getOpportunityDescriptionFragment(opportunity) {
    ...
    if (display == 'short') {
        return html `
      ${setup.DOM.Util.message('(description)', () => renderDescription(opportunity, template.getDescriptionPassage()))}`;
    }
    else {
...
setup.DOM.Card.opportunity = function (opportunity, hide_actions) {
    const template = opportunity.getTemplate();
    const fragments = [];
    fragments.push(setup.DOM.Util.menuItemToolbar(opportunityNameActionMenu(opportunity, hide_actions)));
    // description
    fragments.push(html `
    <div class='textcard'>
  ...
  `);
    ...
    `);
      ...
      `);
        ...
        fragments.push(setup.DOM.create('div', { class: 'opportunitycardoption clear-both' }, inner_fragments));
    }
    if (State.variables.gDebug) {
        fragments.push(html `
      <div>
...
setup.DOM.Card.opportunity_option_selected = function (opportunity) {
    ...
    if (passage) {
        ...
        if (State.variables.gMenuVisible) {
            ...
            setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.link(`Continue`, continue_callback));
            fragments.push(html `
        <div>
