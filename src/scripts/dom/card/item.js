...
setup.DOM.Card.item = function (item, hide_actions) {
    const fragments = [];
    // async here?
    fragments.push(setup.DOM.Util.menuItemToolbar(itemNameActionMenu(item, hide_actions)));
...
setup.DOM.Card.itemcompact = function (item, hide_actions) {
    return setup.DOM.Util.menuItemToolbar(itemNameActionMenu(item, hide_actions));
};
