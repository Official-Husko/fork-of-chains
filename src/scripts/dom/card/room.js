...
export function getUnplacedRoom(template) {
    const unplaced = State.variables.roomlist.getUnplacedRooms().filter(test_room => test_room.getTemplate() == template);
    if (!unplaced.length)
        return null;
    return unplaced[0];
}
...
export function getPlacedRoom(template) {
    const placed = State.variables.roomlist.getRoomInstances({ template: template }).filter(test_room => test_room.isPlaced());
    if (!placed.length)
        return null;
    return placed[0];
}
    ...
    if (!hide_actions &&
        ...
        menus.push(menuItemAction({
            ...
            callback: () => {
                ...
                const reason = State.variables.fortgrid.checkRoomCanRelocateTo(placed, /* new location = */ null, /* skip pathing = */ false);
                if (!reason) {
...
function fullAdjacencyExplanation(room) {
    ...
    const fragments = [];
    const raw_res = State.variables.fortgrid.getAffectingRooms(room, room.getLocation());
...
setup.DOM.Card.roominstance = function (room, hide_actions) {
    ...
    if (State.variables.fort.player.isHasBuilding(setup.buildingtemplate.landscapingoffice)) {
        const explanation = setup.SkillHelper.explainSkills(room.getSkillBonuses(), /* hide skills = */ false, /* to fixed = */ true);
        if (explanation) {
            fragments.push(html `<div>Current effects: ${explanation} ${setup.DOM.Util.message(`(full details)`, () => html `<div class='helpcard'>${fullAdjacencyExplanation(room)}</div>`)}</div>`);
        }
    }
