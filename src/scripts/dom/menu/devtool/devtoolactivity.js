...
function traitFragment(template, is_critical) {
    ...
    inner.push(setup.DOM.Nav.link(`(change)`, () => {
        setup.DevToolHelper.pickTraits(null, /* init traits = */ current).then((selected_traits) => {
            let varname;
            if (is_critical) {
...
function dialogueEditFragment(dialogue, dialogue_index, dialogues) {
    ...
    if (dialogues.length >= 2) {
        remove = setup.DOM.Nav.link(`(delete dialogue sequence #${dialogue_index + 1})`, () => {
    ...
    for (const perkey in dialogue.texts) {
        ...
        if (texts.length >= 2) {
            ...
            for (let i = 0; i < texts.length; ++i) {
                veryin.push(setup.DOM.Nav.link(`(remove alternate text #${i + 1})`, () => {
...
setup.DOM.Menu.devtoolactivity = function (template) {
    ...
    <div>
      ${setup.DOM.Nav.link(`(add new dialogue sequence)`, () => {
        template.dialogues.push(setup.DialogueHelper.createEmptyDialogue(dialogues[dialogues.length - 1].actor));
        refreshPage();
    })}
    </div> 
  `);
  ...
  `);
    ...
    fragments.push(setup.DOM.Util.filterAll({
        ...
        display_callback: (room_template) => {
            ...
            return html `
        <div>
          ${room_template.rep()} ${setup.DOM.Nav.button(`Select`, () => {
