    ...
    if (is_multiple) {
        ...
        `);
                ...
                for (const trait of selected) {
                    traitreps.push(setup.DOM.create('div', { style: "margin: 0 4px; cursor: pointer" }, setup.DOM.Util.onEvent('click', trait.rep(true), callback(trait))));
                }
        ...
        if (finish_callback) {
            button_fragments.push(setup.DOM.Nav.button('Confirm', () => { finish_callback(selected); }));
        }
    ...
    all_fragments.push(setup.DOM.createRefreshable('div', { id: divid2 }, () => {
        return setup.DOM.Util.filterAll({
            ...
            display_callback: (trait) => {
                return setup.DOM.create('div', { style: "cursor: pointer" }, setup.DOM.Util.onEvent('click', selected.includes(trait) ? trait.repPositive(true) : trait.rep(true), callback(trait)));
            },
        });
    }));
 ...
 * @returns {setup.DOM.Node}
 */
setup.DOM.Menu.traitpickermulti = function ({ raw_traits, init_selected, finish_callback, no_overlap, on_change_callback }) {
    return doTraitPick({
