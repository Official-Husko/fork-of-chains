...
setup.DOM.Menu.newgameplus = function () {
    ...
    <div>
      ${setup.DOM.Nav.button(`It's time to retire from leading a slaving company for good`, () => {
        setup.DOM.Helper.replace(`#${CONTAINER_DIV_ID}`, newGamePlusChooseUnitsFragment(/* is new pc = */ true));
    })}
    ...
    <div>
      ${setup.DOM.Nav.button(`Maybe starting a new company might be a good idea...`, () => {
        setup.DOM.Helper.replace(`#${CONTAINER_DIV_ID}`, newGamePlusChooseUnitsFragment(/* is new pc = */ false));
    })}
      ${setup.DOM.Util.help(html `
    ...
    <div>
      ${setup.DOM.Nav.link(`(abandon this idea and return back to your company)`, () => {
...
function newGamePlusChooseUnitsFragment(is_new_pc, selected_units = []) {
    ...
    You can also just ${setup.DOM.Nav.link(`finish selecting units`, () => {
        setup.DOM.Helper.replace(`#${CONTAINER_DIV_ID}`, newGamePlusShowEpilogue(is_new_pc, selected_units));
    })}
    or even ${setup.DOM.Nav.link(`change your mind and go back to your office`, () => {
        setup.DOM.Nav.gotoreturn();
    })}.
    Note that you can only invite units that are ${setup.DOM.Text.successlite('currently at home')}.
  `));
    let candidates = State.variables.company.player.getUnits({}).concat(State.variables.retiredlist.getUnits());
    candidates = candidates.filter(unit => {
        ...
        if (unit.isHasTrait(setup.trait.will_indomitable))
            return false;
        if (selected_units.includes(unit))
            return false;
        if (selected_units.filter(u => u.isSlaver()).length >= slaver_limit && unit.isSlaver())
            return false;
        if (selected_units.filter(u => u.isSlave()).length >= slaves_limit && unit.isSlave())
            return false;
        return true;
    });
        ...
        actions_callback: /** @param {setup.Unit} unit */ (unit) => {
            ...
            menus.push(menuItemAction({
                ...
                callback: () => {
                    ...
                    setup.DOM.Helper.replace(`#${CONTAINER_DIV_ID}`, newGamePlusChooseUnitsFragment(is_new_pc, selected_units));
                },
            }));
            return menus;
        }
...
function newGamePlusShowEpilogue(is_new_pc, selected_units) {
    const fragments = [];
    // describe retirement
    const all_units = State.variables.company.player.getUnits({}).filter(unit => !unit.isYou() && !selected_units.includes(unit));
...
function getNewLeaderText(unit) {
    ...
    let sentences = [];
    sentences.push(setup.Text.replaceUnitMacros([
    ...
    if (Math.random() < setup.NEW_GAME_PLUS_BASE_SUCCESS_CHANCE + success_chance) {
        ...
        if (!options.length) {
            options.push(`It's unclear how a|rep leads the company, however.`, `a|Reps style of leadership remains a mystery, however.`);
        }
    }
    else {
        ...
        if (!options.length) {
            options.push(`It is unclear how a|they failed to lead the company.`, `Nobody knows why a|they failed to lead the company, however.`);
        }
    }
...
export function initNewGamePlus(is_new_pc, selected_units) {
    ...
    for (const slaver of ex_slavers) {
        if (slaver.isSlaver()) {
            setup.qc.AddHistory('unit', `retired from being a slaver at the slaving company known as ${company_name}`).apply(setup.costUnitHelper(slaver));
        }
        else if (slaver.isRetired()) {
            setup.qc.AddHistory('unit', `got invited during retirement for another adventure by the ex-leader of the slaving company known as ${company_name}`).apply(setup.costUnitHelper(slaver));
        }
    }
    setup.qc.AddHistory('unit', `retired from being the leader of the slaving company known as ${company_name}`).apply(setup.costUnitHelper(player));
