...
function unitImageFragment(unit) {
    ...
    <figure class='${div_class}'>
      ...
      <figcaption class='${div_class}'>
        ...
        ${setup.DOM.Nav.link(State.variables.settings.unitimagefull ? `(switch to half-size)` : `(switch to full-size)`, () => {
...
setup.DOM.Menu.unitdescription = function (unit) {
    ...
    { /* Optional paragraph 2 about family members, if any */
        ...
        ${format(`a|They a|is`)}
        ${setup.Text.addCommas(Object.keys(family).map(family_unit_key => {
    ...
    `);
        ...
        ${setup.Text.replaceUnitMacros(`a|They a|is a known bodyshifter, capable of shifting from a|their current a|race body into
            a b|race one.`, { a: unit, b: State.variables.bodyshift.getBody(unit) })}
      `);
        }
    ...
    { /* Current occupations */
      ...
      `);
        ...
        const friendcheck = (unit.isSlaver() &&
