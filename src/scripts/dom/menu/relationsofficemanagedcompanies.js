  ...
  `);
    ...
    if (all_managed_companies.length < max_managed) {
        ...
        for (const companykey in State.variables.company) {
            ...
            if (!State.variables.favor.isCompanyKnown(company))
                continue;
            if (company == State.variables.company.player)
                continue;
            if (all_managed_companies.includes(company))
                continue;
    ...
    setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.move(`Finish [space]`, 'RelationsOffice'));
