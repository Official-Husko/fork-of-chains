...
setup.DOM.Menu.unitdetail = function (unit) {
  ...
  `);
    // actions menu
    outer.push(parseRosterMenuToolbar(unit, getRosterListMenuItems({ unit: unit, hide_details: true })));
    // unit card
    outer.push(html `
  ...
  `);
    ...
    if (State.variables.fort.player.isHasBuilding('armory')) {
        ...
        if (equipment_set) {
            outer.push(html `
      <div>
        ${setup.Text.replaceUnitMacros(`a|Rep a|is currently wearing`, { a: unit })}
        ${equipment_set.rep()}:
        ${actions}
      </div>
    `);
            ...
            if (unit.isSlave()) {
                outer.push(html `
      <div>
        ${setup.Text.replaceUnitMacros(`a|Rep a|is currently naked:`, { a: unit })}
        ${actions}
      </div>
    `);
            }
                ...
                outer.push(html `
      <div>
        ${setup.Text.replaceUnitMacros(`a|Rep a|is currently wearing a|their own basic clothings:`, { a: unit })}
        ${actions}
      </div>
    `);
            }
