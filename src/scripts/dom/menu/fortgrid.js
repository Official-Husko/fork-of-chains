...
setup.FortGridController = class FortGridController extends setup.TwineClass {
     ...
     */
    getElem(location) {
        const translated = State.variables.fortgrid.translateLocationToArrayIndex(location);
    ...
    drawTiles() {
        ...
        // draw top tile just to ensure all cells have the correct width.
        {
            ...
            for (let j = 0; j < State.variables.fortgrid.getWidth(); ++j) {
                top_row.push(setup.DOM.create('td', {
        ...
        for (let i = 0; i < tiles.length; ++i) {
            ...
            for (let j = 0; j < tile_row.length; ++j) {
                ...
                const elem = setup.DOM.create('td', {
                    rowspan: rowspan,
                    colspan: colspan,
        ...
        const table = setup.DOM.create('table', { class: 'fortgrid-table' }, setup.DOM.create('tbody', {}, display_rows));
    ...
    resetRoom() {
        ...
        const reset_tiles = State.variables.fortgrid.relocateRoom(this.getRoom(), this.move_origin_location, 
     ...
     */
    setElem(location) {
        if (State.variables.fortgrid.isOutOfBounds(location))
            return;
        const tile = State.variables.fortgrid.getTile(location);
        const elem = this.getElem(location);
        const room = tile.getRoomInstance();
...
that you can play the game without even bothering with this bonus if you prefer.`;
    ...
    getToolbars() {
        ...
        if (['delete'].includes(this.mode)) {
            setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.link(`Finish removals`, cancel_callback));
        }
          ...
          `);
                ...
                setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.link(html `Finish${cost ? html ` (${setup.DOM.Util.money(-cost)})` : ``}`, return_callback));
            }
        }
        else if (this.mode == 'confirmbuild') {
            setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.link(html `Confirm`, confirm_build_callback));
        }
        const menus = [];
        ...
        extras.push(State.variables.menufilter.getMenuItemChecked('fortgrid', 'show_caption', () => {
            this.refreshAll();
        }, 'Show titles of the rooms at the top left corner of their images'));
        extras.push(State.variables.menufilter.getMenuItemChecked('fortgrid', 'show_tooltip', () => {
            this.refreshAll();
        }, 'Show room tooltips when hovering over them'));
        if (this.is_show_bonus) {
            extras.push(State.variables.menufilter.getMenuItemChecked('fortgrid', 'show_skills', () => {
                this.refreshAll();
            }, 'Show the skill benefits of each room, if any, on the bottom left corner of their images'));
        }
        extras.push(State.variables.menufilter.getMenuItemChecked('fortgrid', 'show_activities', () => {
            this.refreshAll();
        }, 'Show unit activities on the rooms'));
        menus.push(State.variables.menufilter.getMenuFilterToolbarSingleMenu(setup.MenuFilter.getMenus('fortgrid'), 'fortgrid', 'zoom', () => {
            this.refreshAll();
        }));
        if (extras.length) {
...
function on_mouse_enter_callback(tile) {
    return () => {
        ...
        if (['place', 'build'].includes(mode)) {
            ...
            const reason = State.variables.fortgrid.checkRoomCanRelocateTo(room, tile.getLocation(), /* skip pathing = */ true);
            let classname;
            if (!reason) {
...
function on_click_callback(tile) {
    return () => {
        ...
        if (['place', 'build'].includes(mode)) {
            ...
            const reason = State.variables.fortgrid.checkRoomCanRelocateTo(room, tile.getLocation(), /* skip pathing = */ false);
            let tiles;
            if (!reason) {
                // relocate room
                tiles = State.variables.fortgrid.relocateRoom(room, tile.getLocation(), 
            ...
            const room = tile.getRoomInstance();
            if (room && !room.getTemplate().isFixed()) {
                const reason = State.variables.fortgrid.checkRoomCanRelocateTo(room, /* new location = */ null, /* skip pathing = */ false);
                if (!reason) {
...
function get_expansion_row_content(is_indoor) {
    ...
    if (State.variables.gFortGridControl.mode == 'edit') {
        if (State.variables.company.player.getMoney() >= cost) {
            content = setup.DOM.Nav.button(html `Expand ${direction} for ${setup.DOM.Util.money(cost)}`, () => {
                if (is_indoor) {
...
function get_expansion_row(is_indoor) {
    ...
    <tr>
      ...
      >
        ...
        >
          ${setup.DOM.createRefreshable('div', { id: get_expansion_row_refreshable_id(is_indoor) }, () => get_expansion_row_content(is_indoor))}
        </div>
      </td>
    </tr>
  `;
 ...
 * @returns {setup.DOM.Node}
 */
setup.DOM.Menu.fortgrid = function ({ mode, room_key, }) {
    if (State.variables.gFortGridControl) {
        // Resume existing one.
    }
    else {
    ...
    }, () => setup.DOM.Util.menuItemToolbar(State.variables.gFortGridControl.getTitleToolbar())));
    fragments.push(setup.DOM.createRefreshable(`div`, {
        id: FORTGRID_TOOLBAR_DIV_ID,
        class: 'tagtoolbarsticky',
    }, () => setup.DOM.Util.menuItemToolbar(State.variables.gFortGridControl.getToolbars())));
    fragments.push(setup.DOM.create('div', {
        class: 'overflow-story-container',
    }, setup.DOM.createRefreshable(`div`, {
        id: FORTGRID_GRID_DIV_ID,
    }, () => State.variables.gFortGridControl.drawTiles(), true)));
