...
function makeJobDivCallback(unit, job) {
    return () => {
      ...
      `);
            ...
            inner_fragments.push(setup.DOM.Nav.link(html `(Delete) `, () => {
                State.variables.friendship.deleteFriendship(unit, target);
                refreshAll();
            }));
            if (unit.getLover() != target) {
                inner_fragments.push(setup.DOM.Nav.link(html `(Lovers) `, () => {
                ...
                inner_fragments.push(setup.DOM.Nav.link(html `(Breakup) `, () => {
...
setup.DOM.Menu.debugfriendship = function (unit) {
    ...
    for (let i = 0; i < jobs.length; ++i) {
        ...
        outer_fragments.push(setup.DOM.Util.message(`(Friendship with a ${job.getName()}) `, setup.DOM.createRefreshable('div', { id: divname }, makeJobDivCallback(unit, job))));
    }
