...
setup.DOM.Menu.dismissConfirm = function (unit) {
    ...
    <div>
      ...
      ${unit.rep()}?
      ${setup.Text.replaceUnitMacros(`a|They will be gone forever from your company and you will never see a|them ever again.`, { a: unit })}
    </div>
  `);
    fragments.push(html `
    <div>
...
setup.DOM.Menu.retireConfirm = function (unit) {
    ...
    <div>
     ...
     with the help of your ${setup.buildingtemplate.inn.rep()}.`, { a: unit })}
    </div>
  `);
    fragments.push(html `
    <div>
...
setup.DOM.Menu.retiredDismissConfirm = function (unit) {
    ...
    <div>
      ...
      ${setup.Text.replaceUnitMacros(`a|They will be gone forever and you will never see a|them ever again.`, { a: unit })}
    </div>
  `);
    fragments.push(html `
    <div>
      ${setup.DOM.Nav.button(`Yes, I want to REMOVE ${unit.rep()} permanently`, () => {
...
setup.DOM.Menu.rehireRetiredConfirm = function (unit) {
    ...
    <div>
      ...
      ${setup.Text.replaceUnitMacros(`a|They will need ${weeks} weeks to wrap up things on a|their end first before a|they will become available again.`, { a: unit })}
    </div>
  `);
    fragments.push(html `
    <div>
      ${setup.DOM.Nav.button(`Yes, I want to re-hire ${unit.rep()}`, () => {
        ...
        State.variables.leave.leave(unit, `a|is wrapping up ends in order to rejoin your company as a slaver proper`, weeks);
        State.variables.notification.enable();
        setup.notify(`a|Rep a|have rejoined your company! a|They will need ${weeks} weeks in order to wrap up things on a|their end first, however`, { a: unit });
        setup.DOM.Nav.gotoreturn();
    })}
    </div>
  `);
