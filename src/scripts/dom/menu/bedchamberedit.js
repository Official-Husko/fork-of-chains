...
setup.DOM.Menu.bedchamberedit = function (bedchamber) {
    ...
    fragments.push(setup.DOM.Card.bedchamber(bedchamber, /* hide actions = */ false, /* show_remove_button = */ true));
    // show the list of furniture to attach
    /**
     * @type {setup.Furniture[]}
     */
    // @ts-ignore
    const furnitures = State.variables.inventory.getItems().map(item_obj => item_obj.item).filter(item => item instanceof setup.Furniture);
