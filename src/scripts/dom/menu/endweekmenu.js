...
function getInfoMenu() {
    ...
    if (State.variables.calendar && State.variables.unit?.player) {
      ...
      ${State.variables.company.player.getName()} / Week ${State.variables.calendar.getWeek()}
        ...
        // money and prestige
        const moneytexts = [];
        moneytexts.push(html `${setup.DOM.Util.money(State.variables.company.player.getMoney())}`);
        if (State.variables.fort.player.isHasBuilding('recreationwing')) {
            moneytexts.push(html ` / `);
            moneytexts.push(html `${setup.DOM.Util.prestige(State.variables.company.player.getPrestige())}`);
        }
...
function getTopLeftNavigationMenu() {
    ...
    if (State.variables.gMenuVisible) {
        if (State.variables.opportunitylist.isHaveMustAnswerOpportunities()) {
            ...
            fragments.push(setup.DOM.create('span', { class: 'press_on_space' }, setup.DOM.Nav.move(html `ANSWER MAIL`, `OpportunityList`)));
            fragments.push(setup.DOM.Util.help(html `
        ...
        `));
            ...
            if (!deployable) {
                fragments.push(setup.DOM.create('span', { class: 'press_on_space' }, setup.DOM.Nav.link(html `END WEEK`, endWeek)));
            }
            else {
                const menu_id = 'end_week_link_confirm_id';
                const ele = setup.DOM.create('span', { id: menu_id, class: 'press_on_space' }, setup.DOM.Nav.link(html `END WEEK`, () => {
                    setup.DOM.Helper.replace(`#${menu_id}`, setup.DOM.Nav.link(html `REALLY END WEEK`, endWeek));
                }));
                fragments.push(ele);
                fragments.push(html ` `);
                fragments.push(setup.DOM.Text.danger(`[${deployable}]`));
                fragments.push(setup.DOM.create('small', {}, setup.DOM.Util.help(html `You can still send ${deployable} more teams on quests.`)));
            }
        }
    }
    else {
