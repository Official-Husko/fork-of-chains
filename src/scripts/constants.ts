...
// Define the shape of the global setup object for type safety
interface Setup {
  [key: string]: any;
}

// Declare the global setup variable
...
setup.SLAVETRAINVALUE_ADVANCED = Math.round(
  1.2 * 3 * setup.MONEY_PER_SLAVER_WEEK
);
...
setup.COMPUTE_APPROXIMATE_VALUE_MULTIPLIER = 1.1;

// register all constants from different modules
import { registerUnitGenerationConstants } from "./constants/unit-generation";
import { registerDutyConstants } from "./constants/duties";
import { registerMoneyConstants } from "./constants/money";
import { registerRoomConstants } from "./constants/rooms";

registerUnitGenerationConstants(setup);
registerDutyConstants(setup);
registerMoneyConstants(setup);
registerRoomConstants(setup);
